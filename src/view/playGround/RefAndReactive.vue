<template>
  <div>
    <h3>ref和reactive</h3>
    <div>
      <p><span>ref在数组或对象里的值在触发更新后并不会在页面上及时响应</span></p>
    ...
    <div>
      ...
      <div>
        <p>
          <span>使用ref和reactive声明的变量为递归监听，使用shallowRef和shallowReactive声明的变量为非递归监听。</span><br>
          <span>通俗的讲就是ref和reactive创建的对象将会被vue内部进行递归监听，可以监听到对象中的对象是否发生了改变从而更新视图，而shallowRef和shallowReactive创建的对象只能监听到首层对象的变化。</span>
        </p>
      ...
      <h3>triggerRef</h3>
      <div>triggerRef-手动执行与 shallowRef 关联的任何副作用，强制更新视图</div>
      <div>
